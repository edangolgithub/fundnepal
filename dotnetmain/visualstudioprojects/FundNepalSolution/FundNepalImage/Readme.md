...
WORKDIR /src
COPY ["FundNepalImage.csproj", "FundNepalImage/"]
RUN dotnet restore "FundNepalImage/FundNepalImage.csproj"

WORKDIR "/src/FundNepalImage"
COPY . .
RUN dotnet build "FundNepalImage.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "FundNepalImage.csproj" \
...
Execute unit tests
```
    cd "FundNepalImage/test/FundNepalImage.Tests"
    dotnet test
```

Deploy application
```
    cd "FundNepalImage/src/FundNepalImage"
